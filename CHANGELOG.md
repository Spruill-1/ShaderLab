...
### Added
- **Engine-side `Rendering::ReadPixelRegion` helper + `ShaderLabHeadless --pixels` mode.** New `Rendering/PixelReadback.h/.cpp` exposes a host-agnostic pixel-region readback API: `(graph, nodeId, x, y, w, h, dc) → ReadPixelRegionResult { status, pixels, actualWidth, actualHeight }`. Status is one of `Success`/`NotFound`/`NotReady`/`InvalidRegion`/`D2DError`. The existing `MainWindow::ReadPixelRegion` (and therefore the MCP `read_pixel_region` route) now delegates to this helper — ~120 lines of duplicated D2D plumbing collapsed into a 30-line shim. `ShaderLabHeadless` gains `--pixels x,y,w,h` with extension-driven format detection: `.csv` writes `x,y,r,g,b,a` rows, anything else writes packed binary (`uint32 W` + `uint32 H` header + `float[W*H*4]` RGBA row-major). Bypasses the PNG / HdrToneMap path entirely — full-accuracy scRGB samples for MCP-driven dE sweeps and quantitative analysis. Smoke test `Tests/RunHeadlessSmoke.ps1` grew a binary-readback pass that validates exact blob size and header bytes.
- **D2D HdrToneMap pre-pass in ShaderLabHeadless** (default-on). New `--input-peak-nits N` (default 1000), `--output-peak-nits N` (default 80 = SDR), `--no-tonemap` flags. Inserts `CLSID_D2D1HdrToneMap` into the render path before the FP32 readback / PNG encode step so HDR scRGB content above 1.0 doesn't silently saturate to 255 in the output PNG. Display mode picks SDR when output peak ≤ 80 nits, HDR otherwise. The fixed BT.2408-style mid-tone lift D2D applies (decision log #52) is preserved — appropriate for visual inspection. **For quantitative work** (dE sweeps, scRGB pixel measurements), use `--no-tonemap` to bypass the lift, or use the new `--pixels` mode for raw FP32 RGBA. Dropped the JXR / EXR direction entirely: PNG with proper tone mapping is the right answer for visual inspection, and FP16 pixel sampling is now a separate output mode that doesn't go through file encoding.

### Changed
- **Phase 7 v1: `ShaderLabHeadless.exe` console host.** Loads an `.effectgraph` JSON file, renders any node to PNG via the same engine + GraphEvaluator the GUI app uses, no SwapChainPanel / WinUI / DXGI swap chain needed. Unblocks the overnight dE-sweep use case directly: a script can drive the binary across thousands of parameter combinations. Implements the engine ABI version check on startup (matches the GUI app's behavior, surfaces DLL/header mismatches with a clean error). New `ShaderLabHeadless.vcxproj` linked into the engine via project reference; new `Tests/RunHeadlessSmoke.ps1` script wired into CI's `bootstrap-smoke` job. Default MCP port reserved at 47809 (vs 47808 for the GUI app) to avoid shared-machine conflicts. **Not yet implemented (queued):** the full MCP server move from `MainWindow.McpRoutes.cpp` (2500 LoC, 40 routes, ~25 engine-pure / ~12 UI-coupled — needs careful per-route categorization), `--script` JSON batch mode, `.effectgraph` zip archives with embedded media, FP16 pixel-readback for quantitative analysis.
- **Phase 7 spike: headless D3D11+D2D pixel readback (no swap chain).** New `Tests/TestRunner.cpp::TestHeadlessReadback` confirms a host can render a graph and read pixels back using just an ID3D11Device + ID2D1DeviceContext. Phase 7 architecture is unblocked. Lifetime gotcha discovered: `node.cachedOutput` is a non-owning `ID2D1Image*` into the GraphEvaluator's effect cache; the headless host must keep one long-lived `GraphEvaluator` instance alongside the device stack (mirrors the GUI app's `m_evaluator` member pattern). Hoisted the test runner's evaluator to file scope to fix the latent dangling-pointer bug.
...
### Changed
- **Phase 3 follow-up: 10 ShaderLab effects' enum cbuffer fields migrated to `uint` HLSL declarations.** With the typed pack helper in place (commit `c177770`), the historical "declare as float, compare with `> 0.5`" workaround for enum-style cbuffer fields is no longer needed. Migrated cbuffer fields: Luminance Heatmap (`ColormapMode`), Gamut Highlight (`Mode`, `TargetGamut`), Luminance Highlight (`Mode`, `TargetRange`), Gradient Generator (`GradientType`), Delta E Comparator (`Method`, `OutputMode`), Waveform Monitor (`Mode`), Gamut Coverage (`TargetGamut`), Gamut Map (`Mode`, `TargetGamut`, `SourceGamut`), ICtCp Gamut Map (`Mode`, `TargetGamut`, `SourceGamut`), ICtCp Boundary (`TargetGamut`). Existing threshold comparisons in shader bodies (`Mode > 0.5 && Mode < 1.5` etc.) still work — HLSL promotes uint to float for literal comparisons, so behavior is identical. Cleanup of the comparisons themselves into clean `Mode == 1` form is intentionally deferred (high edit churn for zero behavior change). `ParameterDefinition::typeName` stays `L"float"` because it drives the Properties-panel UI, not the cbuffer pack — the typed pack helper does float→uint conversion at write time. Effect versions bumped on each migrated effect (10 bumps total).
